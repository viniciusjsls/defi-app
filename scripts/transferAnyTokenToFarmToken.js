...
module.exports = async function (callback) {
  ...
  const allowanceBefore = await anyToken.allowance(
    accounts[0],
    farmToken.address
  )
  console.log(
    "Amount of AnyToken FarmToken is allowed to transfer on our behalf Before: " +
      allowanceBefore.toString()
  )

  await anyToken.approve(farmToken.address, web3.utils.toWei("100", "ether"))

  const allowanceAfter = await anyToken.allowance(accounts[0], farmToken.address)
  console.log(
    "Amount of AnyToken FarmToken is allowed to transfer on our behalf After: " +
